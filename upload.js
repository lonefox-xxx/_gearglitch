...
async function handleUpload() {
    ...
    for (const { id, videoUrl, musicInfo, hashtags, caption, shortCode, videoDuration, isSponsored } of reels) {
        console.log(`proccesing ${id}`)

        if (isSponsored) continue

        try {
            const { result } = await sendVideoFromUrl(videoUrl)


            const reelRef = JSON.parse(fs.readFileSync('./reels_ref.json', 'utf-8') || '[]') || []
            reelRef.push({ id, musicInfo, hashtags, caption, shortCode, videoDuration, ...result, })
